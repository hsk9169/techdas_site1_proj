...
class tag_desc:
    ...
    def get_sensor_data():
        query = cur.execute(f""" \
                ...
                OR var = 'X13' OR var = 'X15' OR var = 'X17' OR var = 'X19' \
                OR var = 'X21' OR var = 'X22' OR var = 'X24' OR var = 'X25' \
                OR var = 'X27' OR var = 'X28' OR var = 'X30' OR var = 'X33' \
                OR var = 'X35' OR var = 'X36' OR var = 'X37' OR var = 'X38' \
                OR var = 'X39' OR var = 'X49' OR var = 'X50' \
                """)
...
class raw_data:
    def get_air_ratio(filter_date):
        query = cur.execute(f""" \
                SELECT DateTime, X40, X41, X42, X43, X44, X45, X46, X47 FROM raw_data \
                WHERE DateTime LIKE '{filter_date}' \
                """)
    ...
    def get_sv_value(filter_date):
        query = cur.execute(f""" \
                SELECT DateTime, X61 FROM raw_data \
                WHERE DateTime LIKE '{filter_date}' \
                """)
    ...
    def get_sensor(filter_date):
        query = cur.execute(f""" \
                SELECT DateTime, X3, X5, X7, X9, X13, X15, X17, X19, X21, X22, X24, X25, X27, X28, X30, X33, X35, X36, X37, X38, X39, X49, X50 FROM raw_data \
                WHERE DateTime LIKE '{filter_date}' \
                """)
        ...
        data['X7'] = float(row[3])
        data['X9'] = float(row[4])
        data['X13'] = float(row[5])
        data['X15'] = float(row[6])
        data['X17'] = float(row[7])
        data['X19'] = float(row[8])
        data['X21'] = float(row[9])
        data['X22'] = float(row[10])
        data['X24'] = float(row[11])
        data['X25'] = float(row[12])
        data['X27'] = float(row[13])
        data['X28'] = float(row[14])
        data['X30'] = float(row[15])
        data['X33'] = float(row[16])
        data['X35'] = float(row[17])
        data['X36'] = float(row[18])
        data['X37'] = float(row[19])
        data['X38'] = float(row[20])
        data['X39'] = float(row[21])
        data['X49'] = float(row[22])
        data['X50'] = float(row[23])
        return data
